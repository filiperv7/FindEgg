<template>
  <div class="tela-cadastro">
    <v-app >
      ...
      v-model="select"
      row
      :rules="[(v) => !!v || 'É necessário escolher uma das opções']"
    >
      <v-radio
        label="Comprador"
        value="comprador"
      ...
      ></v-radio>
          ...
          <v-checkbox
            v-model="checkbox"
            :rules="[
              (v) => !!v || 'Você precisa aceitar os termos e condições!',
            ]"
...
export default {
  ...
  data: () => ({
    ...
    valid: true,
    name: "",
    nameRules: [(v) => !!v || "Nome não foi inserido"],
    endereco: "",
    enderecoRules: [(v) => !!v || "Endereço não foi inserido"],
    email: "",
    emailRules: [
